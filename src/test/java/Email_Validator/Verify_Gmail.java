...
public class Verify_Gmail {
        ...
        try {
            ...
            for (int n = messages.length, i = n - 1; i > n - 6; i--) {
                ...
                if (message.getSubject().contains("Thank You For Your Order")) {
                    ...
                    if(printPlainTextContent(message).equals("true")){
                    	break;
                    }
                }
    ...
    private static String printPlainTextContent(Message message) throws Exception {
        ...
        if (content instanceof String) {
            ...
            if (mimeType.startsWith("text/plain")) {
                ...
                if (matcher.find()) {
                    ...
                    log.info("OrderNumber from EMail= " +EmailOrderNumber+"\n");
                    if(EmailTrimmedOrderNumber.equalsIgnoreCase(BrowserorderNumberTexts)){
                        log.info("Order ID's Matched in Both UI and EMAIL\n");
                        status = "true";
                    }else{
                    	log.info("Order ID's DID Matched in Both UI and EMAIL\n");
                        status = "false";
                       // throw new Exception();
                    }

                } else {
                	log.info("Order number not found in the email body.\n");
                    status = "false";
                }
            ...
            for (int i = 0; i < multipart.getCount(); i++) {
                ...
                if (bodyPart.isMimeType("text/plain")) {
                    ...
                    if (matcher.find()) {
                        ...
                        log.info("OrderNumber from EMail= " +MyOrdereNumber+"\n");
